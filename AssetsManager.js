...
AssetsManager.prototype.loadImage = function (key, url) {
    ...
    imagem.addEventListener("load", function () {
        that.carregadas++;
        console.log(`imagem ${that.carregadas}/${that.aCarregar} ${key}: ${url} carreganda!`);
    })
}
...
AssetsManager.prototype.loadAudio = function (key, url) {
    ...
    /*audio.addEventListener("load", function () {
        that.carregadas++;
        console.log(`Audio ${that.carregadas}/${that.aCarregar} ${key}: ${url} carreganda!`);
    })
    */
}
...
AssetsManager.prototype.play = function (key) {
    if (!this.audios[key]){
        throw new Erro (`chave de audio invalida: ${key}!`)
    }
    for (var i = 0; i < this.MAX_CHANNELS; i++){
        var agora = new Date();
